  ...
  <div>
      ...
      <table class="table table-striped" aria-describedby="productos">
        <thead>
          <tr>
            ...
            <th scope="row" @click="changeOrder('fechaActualizacion')">
              <span>Fecha Actualizacion</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'fechaActualizacion'"></jhi-sort-indicator>
            </th>
            ...
            <th scope="row" @click="changeOrder('categoriaProducto.id')">
              <span>Categoria Producto</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'categoriaProducto.id'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="producto in productos" :key="producto.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="producto.categoriaProducto">
                <router-link :to="{ name: 'CategoriaProductoView', params: { categoriaProductoId: producto.categoriaProducto.id } }">{{
                  producto.categoriaProducto.id
                }}</router-link>
              </div>
            </td>
            <td class="text-right">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="autolavadoSaasApp.producto.delete.question" data-cy="productoDeleteDialogHeading">Confirmar operación de borrado</span>
      </template>
